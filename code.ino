...
// don't change
const float defaultFrequency = 104.5;
...
RadioStation stations[] = {
  ...
  { 94.8, "RTL 2" },
  { 96.6, "FRANCE INTER" },
  { 98.8, "FRANCE BLEU LORRAINE NORD" },
  { 101.3, "RCF JERICO MOSELLE" },
  { 102.3, "EUROPE 2 LORRAINE" },
  { 102.9, "RADIO MELODIE" },
  { 104.5, "RIRE & CHANSONS" },
  { 106.0, "EUROPE 1" },
  { 106.4, "FRANCE INFO" }
...
void setup() {
  ...
  for (int i = 0; i < numStations; i++) {
    if (frequency == stations[i].frequency) {
      display.setTextSize(1);
      display.setCursor(0, 0);
      display.print(stations[i].name);
      break;
    }
  }
...
void btnAction(int stateLeft, int stateCenter, int stateRight, float& frequency) {
  if (stateCenter && stateRight) {
    // Trouver la station suivante la plus proche
    for (int i = 0; i < numStations; i++) {
      if (stations[i].frequency > frequency) {
        frequency = stations[i].frequency;
        delay(500);
        break;
      }
    }
  } else if (stateCenter && stateLeft) {
    // Trouver la station précédente la plus proche
